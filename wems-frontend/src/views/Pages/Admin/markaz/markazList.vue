<template>
  ...
  <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
    ...
    <div>
      ...
      <select v-model="filters.center_id" class="w-full px-3 py-2 border border-gray-300 rounded-sm focus:ring-2 focus:ring-indigo-500 focus:border-transparent bg-gray-50">
        <option value="">সকল কেন্দ্র</option>
        <option v-for="center in availableCenters" :key="center.id" :value="center.id">{{ center.name }}</option>
      </select>
    </div>
    <!-- Status Filter -->
    <div>
  ...
  <div class="mt-4">
    ...
    <div v-if="showAdvancedFilters" class="mt-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
      ...
      <!-- Data Table with Smart Scrolling -->
  <div class="bg-white rounded-sm shadow border border-gray-300">
        <div class="table-wrapper" :class="{ 'scrollable': filteredData.length > 50 }">
          <table class="min-w-full divide-y divide-gray-300">
            <thead class="bg-gray-200 rounded-t-sm">
              <tr>
            ...
            <tbody class="bg-white text-xl divide-y divide-gray-200">
              <tr v-for="(item, $index) in paginatedData" :key="item.id" class="hover:bg-gray-50 transition-colors"
                ...
                <td v-for="column in visibleColumns" :key="column.key" class="px-6 py-4 whitespace-nowrap text-base text-gray-900">
                  ...
                  <template v-else-if="column.key === 'center_id'">
                    <span class="bg-purple-100 text-purple-800 px-2 py-1 inline-flex text-md leading-5 font-semibold rounded-sm">
                      {{ item.center_name_display || `কেন্দ্র ${item.center_id}` }}
                    </span>
                  </template>
