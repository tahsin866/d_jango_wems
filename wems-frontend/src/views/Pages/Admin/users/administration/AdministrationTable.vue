<template>
  <div style="font-family: 'SolaimanLipi', sans-serif;">
    <!-- Debug Info for AdministrationTable -->
    <div class="mb-2 p-2 bg-red-50 rounded text-xs text-red-600">
      <strong>প্রশাসন বিভাগ টেবিল লোড হয়েছে</strong> - Total Users: {{ users.length }}
    </div>
      ...
      <table class="min-w-full border border-gray-200 rounded-lg shadow-sm bg-white">
        <thead>
          <tr class="bg-gray-100">
            ...
            <th class="px-4 py-2 text-left font-semibold text-gray-700">প্রশাসন পজিশন</th>
    ...
    <div class="mt-4">
      <button v-if="canCreate" @click="$emit('create')" class="bg-green-500 hover:bg-green-600 text-white px-4 py-3 rounded-sm shadow text-sm font-semibold">নতুন প্রশাসন ইউজার</button>
    </div>
  </div>
</template>
...
const dummyUsers: User[] = [
  { id: 24, name: 'Administration Head', email: 'adminhead@erp.com', role: 5, position: 'Head of Administration', status: 'active' },
  { id: 25, name: 'HR Manager', email: 'hrmanager@erp.com', role: 6, position: 'HR Manager', status: 'active' },
  { id: 26, name: 'Office Manager', email: 'officemanager@erp.com', role: 6, position: 'Office Manager', status: 'active' },
  { id: 27, name: 'Secretary', email: 'secretary@erp.com', role: 6, position: 'Secretary', status: 'active' },
  { id: 28, name: 'Administrative Assistant', email: 'adminassist@erp.com', role: 6, position: 'Administrative Assistant', status: 'active' }
]
