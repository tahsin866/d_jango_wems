<template>
  <div style="font-family: 'SolaimanLipi', sans-serif;">
    <!-- Debug Info for AccountsTable -->
    <div class="mb-2 p-2 bg-blue-50 rounded text-xs text-blue-600">
      <strong>একাউন্টস টেবিল লোড হয়েছে</strong> - Total Users: {{ users.length }}
    </div>
      ...
      <table class="min-w-full border border-gray-200 rounded-lg shadow-sm bg-white">
        <thead>
          <tr class="bg-gray-100">
            ...
            <th class="px-4 py-2 text-left font-semibold text-gray-700">অ্যাকাউন্টস পজিশন</th>
    ...
    <div class="mt-4">
      <button v-if="canCreate" @click="$emit('create')" class="bg-green-500 hover:bg-green-600 text-white px-4 py-3 rounded-sm shadow text-sm font-semibold">নতুন অ্যাকাউন্টস ইউজার</button>
    </div>
  </div>
</template>
...
const dummyUsers: User[] = [
  { id: 1, name: 'Accounts Head', email: 'accountshead@erp.com', role: 5, position: 'Head of Accounts', status: 'active' },
  { id: 2, name: 'Senior Accountant', email: 'senioracc@erp.com', role: 6, position: 'Senior Accountant', status: 'active' },
  { id: 3, name: 'Junior Accountant', email: 'junioracc@erp.com', role: 6, position: 'Junior Accountant', status: 'active' },
  { id: 4, name: 'Cashier', email: 'cashier@erp.com', role: 6, position: 'Cashier', status: 'active' },
  { id: 5, name: 'Accounts Assistant', email: 'accassist@erp.com', role: 6, position: 'Accounts Assistant', status: 'active' }
]
