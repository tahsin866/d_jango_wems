<template>
  <div style="font-family: 'SolaimanLipi', sans-serif;">
    <!-- Debug Info for PublicationsTable -->
    <div class="mb-2 p-2 bg-green-50 rounded text-xs text-green-600">
      <strong>পাবলিকেশন টেবিল লোড হয়েছে</strong> - Total Users: {{ users.length }}
    </div>
      ...
      <table class="min-w-full border border-gray-200 rounded-lg shadow-sm bg-white">
        <thead>
          <tr class="bg-gray-100">
            ...
            <th class="px-4 py-2 text-left font-semibold text-gray-700">পাবলিকেশন পজিশন</th>
    ...
    <div class="mt-4">
      <button v-if="canCreate" @click="$emit('create')" class="bg-green-500 hover:bg-green-600 text-white px-4 py-3 rounded-sm shadow text-sm font-semibold">নতুন পাবলিকেশন ইউজার</button>
    </div>
  </div>
</template>
...
const dummyUsers: User[] = [
  { id: 5, name: 'Publication Head', email: 'pubhead@erp.com', role: 5, position: 'Head of Publications', status: 'active' },
  { id: 6, name: 'Senior Editor', email: 'senioreditor@erp.com', role: 6, position: 'Senior Editor', status: 'active' },
  { id: 7, name: 'Graphic Designer', email: 'designer@erp.com', role: 6, position: 'Graphic Designer', status: 'active' },
  { id: 8, name: 'Content Writer', email: 'writer@erp.com', role: 6, position: 'Content Writer', status: 'active' },
  { id: 9, name: 'Proofreader', email: 'proofreader@erp.com', role: 6, position: 'Proofreader', status: 'active' },
  { id: 10, name: 'Publication Assistant', email: 'pubassist@erp.com', role: 6, position: 'Publication Assistant', status: 'active' }
]
