<template>
  <div style="font-family: 'SolaimanLipi', sans-serif;">
    <!-- Debug Info for TrainingTable -->
    <div class="mb-2 p-2 bg-orange-50 rounded text-xs text-orange-600">
      <strong>প্রশিক্ষণ শাখা টেবিল লোড হয়েছে</strong> - Total Users: {{ users.length }}
    </div>
      ...
      <table class="min-w-full border border-gray-200 rounded-lg shadow-sm bg-white">
        <thead>
          <tr class="bg-gray-100">
            ...
            <th class="px-4 py-2 text-left font-semibold text-gray-700">প্রশিক্ষণ পজিশন</th>
    ...
    <div class="mt-4">
      <button v-if="canCreate" @click="$emit('create')" class="bg-green-500 hover:bg-green-600 text-white px-4 py-3 rounded-sm shadow text-sm font-semibold">নতুন প্রশিক্ষণ ইউজার</button>
    </div>
  </div>
</template>
...
const dummyUsers: User[] = [
  { id: 29, name: 'Training Head', email: 'traininghead@erp.com', role: 5, position: 'Head of Training', status: 'active' },
  { id: 30, name: 'Senior Trainer', email: 'seniortrainer@erp.com', role: 6, position: 'Senior Trainer', status: 'active' },
  { id: 31, name: 'Trainer', email: 'trainer@erp.com', role: 6, position: 'Trainer', status: 'active' },
  { id: 32, name: 'Assistant Trainer', email: 'assisttrainer@erp.com', role: 6, position: 'Assistant Trainer', status: 'active' },
  { id: 33, name: 'Training Coordinator', email: 'trainingcoord@erp.com', role: 6, position: 'Training Coordinator', status: 'active' }
]
