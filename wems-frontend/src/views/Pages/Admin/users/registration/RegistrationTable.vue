<template>
  <div style="font-family: 'SolaimanLipi', sans-serif;">
    <!-- Debug Info for RegistrationTable -->
    <div class="mb-2 p-2 bg-indigo-50 rounded text-xs text-indigo-600">
      <strong>রেজিস্ট্রেশন শাখা টেবিল লোড হয়েছে</strong> - Total Users: {{ users.length }}
    </div>
      ...
      <table class="min-w-full border border-gray-200 rounded-lg shadow-sm bg-white">
        <thead>
          <tr class="bg-gray-100">
            ...
            <th class="px-4 py-2 text-left font-semibold text-gray-700">রেজিস্ট্রেশন পজিশন</th>
    ...
    <div class="mt-4">
      <button v-if="canCreate" @click="$emit('create')" class="bg-green-500 hover:bg-green-600 text-white px-4 py-3 rounded-sm shadow text-sm font-semibold">নতুন রেজিস্ট্রেশন ইউজার</button>
    </div>
  </div>
</template>
...
const dummyUsers: User[] = [
  { id: 20, name: 'Registration Head', email: 'reghead@erp.com', role: 5, position: 'Head of Registration', status: 'active' },
  { id: 21, name: 'Registration Officer', email: 'regofficer@erp.com', role: 6, position: 'Registration Officer', status: 'active' },
  { id: 22, name: 'Admission Coordinator', email: 'admissioncoord@erp.com', role: 6, position: 'Admission Coordinator', status: 'active' },
  { id: 23, name: 'Student Counselor', email: 'counselor@erp.com', role: 6, position: 'Student Counselor', status: 'active' }
]
