    ...
    def create(self, validated_data):
        ...
        if validated_data.get('madrasha_name') and validated_data.get('village'):
            try:
                ...
                if school:
                    school_id = school.id  # Use school.id (primary key)
                    print(f"Found school: {school.mname}, school_id: {school_id}")
        ...
        user_info_data = {
            'user': user,
            'madrasha_id': school_id,  # Maps to 'id' column in database via db_column
